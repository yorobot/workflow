...
SportDb::Model::Event.order( :id ).each do |event|
  ...
  next unless event.league.key.start_with?( league )

  #######
  ## some special cases (auto-) skipping for now e.g. playoffs, relegation, etc.
  ...
  path = "./#{reponame}.csv/#{season_path}/#{basename}.csv"
  puts "  path=>#{path}<"
